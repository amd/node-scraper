...
def test_invalid_data(mock_analyzer, dummy_data_model):
    @analyze_decorator
    ...
    assert result.message == "Invalid data input"
...
def test_success_no_args(mock_analyzer, dummy_data_model, dummy_arg):
    @analyze_decorator
...
def test_success_with_args(mock_analyzer, dummy_data_model, dummy_arg):
    @analyze_decorator
    ...
    assert result.status == ExecutionStatus.EXECUTION_FAILURE
    assert any(
        "Exception during data analysis: some_err" in event["description"]
